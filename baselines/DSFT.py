...
def dsft_forward_process(
    ...
    is_num = torch.zeros_like(batch, dtype=torch.bool, device=device)
    for i in range(b):
        for pos in range(L):
            if answer_mask[i, pos]:
                try:
                    s = tokenizer.decode(batch[i, pos]).strip()
                    if all(c in "0123456789.-+%" for c in s):
                        is_num[i, pos] = True
                except:
                    pass

    mask_index = (torch.rand_like(batch.float()) < t) & answer_mask
    num_extra_mask = is_num & (torch.rand_like(is_num.float()) < (t / num_weight))
    mask_index = mask_index | num_extra_mask

    for i in range(b):
        ans_pos = torch.where(answer_mask[i])[0]
        if len(ans_pos) < min_span:
            continue
        masked_pos = torch.where(mask_index[i, ans_pos])[0]
        if len(masked_pos) > 0:
            s_idx = masked_pos[torch.randint(0, len(masked_pos), (1,))]
            span_len = torch.randint(min_span, max_span + 1, (1,), device=device)
            s = ans_pos[s_idx]
            e = min(s + span_len, ans_pos.max() + 1)
            mask_index[i, s:e] = True

    for i in range(b):
        ans = answer_mask[i]
        if mask_index[i, ans].all():
            valid = torch.where(ans)[0]
            mask_index[i, valid[torch.randint(0, len(valid), (1,))]] = False

    p_mask = torch.full((b, L), t, device=device)
    p_mask = torch.where(answer_mask, p_mask, 0.0)
    p_mask = torch.where(is_num, t / num_weight, p_mask)

    noisy_batch = torch.where(mask_index, mask_id, batch)

    return noisy_batch, mask_index, p_mask
    ...
    for epoch in range(Config.EPOCHS):
        ...
        for batch in progress_bar:
            ...
            for mc in range(Config.MC_NUM):
                noisy_batch, mask_index, p_mask = dsft_forward_process(
                    input_ids,
                    prompt_lengths,
                    Config.MASK_ID,
                    tokenizer
            ...
            if (global_step + 1) % Config.GRADIENT_ACCUMULATION_STEPS == 0:
                ...
                loss_list.append(min(accum_loss / Config.GRADIENT_ACCUMULATION_STEPS, 3))
                accum_loss = 0
        ...
        print(epoch, epoch_loss_sum / max(epoch_token_count, 1))
