...
def db_forward_process(batch, prompt_lengths, mask_id, t0_logit):
    b, l = batch.shape
    device = batch.device
    eps = 1e-8
    ...
    n_tokens = answer_mask.sum(dim=1, keepdim=True)
    H_tilde = 1 - sum_H / (n_tokens * H + eps)

    t = torch.rand(1, device=device).item()

    lambda_val = 1.0
    t_tensor = torch.tensor(t, device=device, dtype=torch.float32)
    S_t = lambda_val * torch.sin(t_tensor * torch.pi)

    mask_prob = t + S_t * H_tilde
...
def train():
    ...
    for epoch in range(Config.EPOCHS):
        ...
        for batch in progress_bar:
            ...
            for mc in range(Config.MC_NUM):
                ...
                noisy_batch, mask_index, p_mask = db_forward_process(
                    input_ids,
                    prompt_lengths,
                    Config.MASK_ID,
                    t0_logits
    ...
    final_save_path = os.path.join(SAVE_DIR, "final_lora_model")

    model.save_pretrained(final_save_path)
    tokenizer.save_pretrained(final_save_path)

    print(f"Training finished! Final LoRA adapter saved to {final_save_path}")
