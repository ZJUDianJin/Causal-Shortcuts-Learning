...
def load_on_gpu(memory_in_mb: int):
    num_gpus = torch.cuda.device_count()
    tensors = []
    for i in range(num_gpus):
        with torch.cuda.device(i):
            num_elements = memory_in_mb * 1024 * 1024 // 4
            t = torch.empty(num_elements, dtype=torch.float32, device=f"cuda:{i}")
            tensors.append(t)
            print(f"Allocated {memory_in_mb}MB on cuda:{i}")
    return tensors
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, choices=["gsm8k", "math", "humaneval", "mbpp", "sudoku", 'gpqa', 'sat','arc_c','mmlu_stem'], default="gsm8k"
    )
    ...
    num_evals = {"gsm8k": -1, "math": -1, "humaneval": -1, "mbpp": -1, "sudoku": -1,'gpqa':-1, 'sat':-1,'arc_c':-1,'mmlu_stem':-1}
